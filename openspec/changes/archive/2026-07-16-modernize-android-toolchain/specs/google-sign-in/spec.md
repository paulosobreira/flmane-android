...
<!-- No requirement text changes. This change makes the implementation actually conform to these
     already-existing requirements (the current source tree still uses the legacy GoogleSignInClient
     flow they were written to replace); the requirement text itself is carried over unchanged. -->
